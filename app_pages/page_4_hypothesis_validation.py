...
def page4_hypothesis_validation_body():
    ...
    st.error(
        f"**False**. \n"
        f"Our study showed that the houses with the highest sale prices are "
        f"not those in the best condition. \n"
        f"Houses with the best rating for `OverallCond` have not sold for "
        f"over 500,000 USD. Many houses with an `OverallCond` rating of 5 "
        f"have a sale price in excess of 500,000 USD, more expensive than "
        f"all houses with the highest condition ratings. \n"
        f"A minimum `OverallCond` rating of 5 is necessary to achieve the "
        f"highest sale prices."
        )
